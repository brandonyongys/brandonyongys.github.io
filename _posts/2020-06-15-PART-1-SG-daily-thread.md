...
Above is the number of psots per user for each daily thread since 2014. The average number of posts per user back then was about 1 to 2 posts and it has grow up to 3 to 4 posts in recent time. Occasionally the number of posts per user does go up to 5 and beyond. You can see a clear difference in number of posts per user on weekdays versus weekends. Users tend to post about 3 to 4 posts on weekdays and on weekends, they tend to post about 3 to 4. This is very likely due to fewer engagements as users are enjoying their life offline.
